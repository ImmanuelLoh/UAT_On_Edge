"""
face_sensor_optimized.py
PASO Benchmark: OPTIMIZED — CameraStream threaded + 30 FPS cap.

Same threaded camera as face_sensor_after.py, but the pipeline is throttled
to 30 FPS via a frame-budget sleep. This prevents FaceMesh from running at
~84 FPS and burning unnecessary CPU cycles beyond what a UAT session needs.

Expected vs AFTER:
  - FPS       : ~30 (capped, vs ~84 uncapped)
  - CPU       : ~11-13% (vs ~31%)
  - Latency   : ~same per frame (FaceMesh cost is fixed per inference)
  - Capture   : still <1ms (background thread keeps buffering)

Run:
    python3 rpi_a/benchmarks/face_sensor_optimized.py
...
from rpi_a.sensors.utils.CameraStream import CameraStream
...
_TARGET_FPS = 30
_FRAME_BUDGET = 1.0 / _TARGET_FPS   # 33.33ms per frame
...
# ---------------------------------------------------------------------------
# FACE SENSOR (uses CameraStream — threaded, non-blocking reads)
# ---------------------------------------------------------------------------
class FaceSensor:
    def __init__(self, screen_w: int, screen_h: int, debug: bool = False):
        ...
        self._stream = CameraStream(0, width=_WIDTH, height=_HEIGHT)
...
if __name__ == "__main__":
    ...
    print(f"[Benchmark] Measuring for {MEASURE}s (capped at {_TARGET_FPS} FPS)...")
    ...
    while True:
        ...
        # --- 1. Camera read (NON-BLOCKING: returns instantly from buffer) ---
        ...
        # --- 6. FPS cap: sleep for remainder of frame budget ---
        t_work_end = time.perf_counter()
        work_elapsed = t_work_end - t_frame_start
        sleep_s = _FRAME_BUDGET - work_elapsed
        if sleep_s > 0:
            time.sleep(sleep_s)
        sleep_ms = max(0.0, sleep_s) * 1000

        # --- Total (includes sleep) ---
        ...
        lat_sleep_list.append(sleep_ms)
    ...
    # ---------------------------------------------------------------------------
    print("\n" + "=" * 63)
    print("  BENCHMARK SUMMARY — OPTIMIZED (CameraStream + 30 FPS cap)")
    print("  Full pipeline: HeadPose + EyeAnalytics + FaceAnalytics")
    print("=" * 63)
    print(f"Resolution : {_WIDTH}x{_HEIGHT} @ {_TARGET_FPS} FPS (capped)")
    print(f"Camera     : CameraStream (background thread, non-blocking read)")
    ...
    if fps_list:
        ...
        print(f"\nNOTE: Sleep (FPS cap) is idle time — CPU is not active during this.")
        print(f"      FaceMesh inference remains the dominant active cost.")
        print(f"      Compare with face_sensor_after.py to see CPU reduction.")
    else:
