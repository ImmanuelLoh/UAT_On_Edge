"""
face_sensor_after.py
PASO Benchmark: AFTER optimisation.

Full production pipeline (HeadPose + EyeAnalytics + FaceAnalytics) with
CameraStream: background thread buffers frames continuously.
Camera read() returns immediately from the buffer - no per-frame stall.
...
if __name__ == "__main__":
    ...
    while True:
        ...
        if results.multi_face_landmarks:
            ...
            #     cv2.putText(frame, f"[AFTER] CameraStream threaded", (10, 82), _FONT, 0.5, (0, 255, 100), 2)
    ...
    # ---------------------------------------------------------------------------
    print("\n" + "=" * 55)
    print("  BENCHMARK SUMMARY — AFTER (CameraStream threaded)")
    ...
    if fps_list:
        ...
        modules = [
            ...
            ("  Camera capture", lat_capture_list),   # KEY: should be <1ms now
            ("  FaceMesh infer", lat_facemesh_list),  # now the dominant cost
        ...
        print("\nNOTE: Camera capture should be <1ms (buffer copy only).")
        print("      FaceMesh inference is now the dominant bottleneck.")
        print("      Compare with face_sensor_before.py to see the delta.")
    else:
